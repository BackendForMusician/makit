...
//                                FieldRepository fieldRepository, GenreRepository genreRepository) {
...
//        project.setUser(loginMember);
//        project.setTitle(request.getTitle());
//        project.setDescription(request.getDescription());
//        project.setMusician(request.isMusician());
//
...
public class ProjectUploadService {
    ...
    public ProjectUploadResponseDTO saveProject(ProjectUploadRequestDTO request, UserEntity loginMember, List<MultipartFile> files) {
        ...
        if (!request.isMusician()) {
            long existingProjectCount = projectRepository.countByUser(loginMember);
            if (existingProjectCount > 0) {
                return new ProjectUploadResponseDTO(false, "뮤지션 등록을 하지 않은 경우, 하나의 프로젝트만 업로드할 수 있습니다.");
            }
        }
